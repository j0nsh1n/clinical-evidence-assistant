...
def search(
    q: str = Query(..., min_length=1, description="Search terms (PubMed query syntax allowed)."),
    max_results: int = Query(20, ge=1, le=50, description="Maximum number of results."),
) -> SearchResponse:
    """Search PubMed and return article summaries, each with an evidence-level hint."""
    query = q.strip()
    if not query:
        raise HTTPException(status_code=422, detail="Provide a non-empty search query.")
    try:
        results = evidence_service.search(query, max_results=max_results)
